...
class CvController {
  static async validateCv(cv) {
    if (cv.experiences.length !== 0) {
      for (let i of cv.experiences) {
        ...
        if (isNaN(start))
          throw {
            name: "BadRequest",
            message: "Invalid experience start date",
          };
        if (isNaN(end))
          throw { name: "BadRequest", message: "Invalid experience end date" };
    ...
    if (cv.educations.length !== 0) {
      for (let i of cv.educations) {
        if (!i?.degree)
          throw { name: "BadRequest", message: "Degree is required" };
        if (!i?.institution)
        ...
        if (isNaN(start))
          throw { name: "BadRequest", message: "Invalid education start date" };
        if (isNaN(end))
          throw { name: "BadRequest", message: "Invalid education end date" };

        if (end - start < 0)
          throw {
            name: "BadRequest",
            message: "Education end date cannot be more recent than start date",
          };
        if (start > now)
          throw {
            name: "BadRequest",
  ...
  static async createCv(req, res, next) {
    try {
      ...
      if (user.quota < 5)
        throw { name: "Forbidden", message: "Not enough tokens to create CV" };
      ...
      await cv.reload();
      res.status(201).json({ message: "Create CV Success!" });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async updateCv(req, res, next) {
    try {
      ...
      if (!user) throw { name: "NotFound", message: "User not found" };
      if (user.quota < 5)
        throw { name: "Forbidden", message: "Not enough tokens to edit CV" };
